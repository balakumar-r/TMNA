...
function createSearchIcon() {
    return `
    <svg class="select-vehicle-dropdown-search-icon" width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
function setInvalid(select, isInvalid) {
    ...
    const field = select.closest('.select-vehicle-dropdown-field');
...
export default function decorate(block) {
    ...
    const uniqueId = `select-vehicle-dropdown-${Math.random().toString(36).slice(2, 8)}`;
    const yearId = `${uniqueId}-year`;
    const modelId = `${uniqueId}-model`;

    block.innerHTML = `
    <form class="select-vehicle-dropdown-form" novalidate>
      <div class="select-vehicle-dropdown-form-body">
        <div class="select-vehicle-dropdown-field select-vehicle-dropdown-field-year">
          <label class="select-vehicle-dropdown-label" for="${yearId}">
            Year
            <span class="select-vehicle-dropdown-required">*</span>
          </label>

          <div class="select-vehicle-dropdown-select-wrap">
            <select id="${yearId}" name="year" required aria-label="Year"></select>
          </div>
        </div>

        <div class="select-vehicle-dropdown-field select-vehicle-dropdown-field-model">
          <label class="select-vehicle-dropdown-label" for="${modelId}">
            Model
            <span class="select-vehicle-dropdown-required">*</span>
          </label>
        ...
        <button class="select-vehicle-dropdown-submit" type="submit" disabled>
          ${createSearchIcon()}
          <span>${escapeHtml(buttonText)}</span>
        </button>
      </div>

      <div class="select-vehicle-dropdown-footer">
        <span>${escapeHtml(signInTextBefore)}</span>
        "<a class="select-vehicle-dropdown-signin" href="${escapeHtml(signInUrl)}">${escapeHtml(signInLabel)}</a>
        <span>${escapeHtml(signInTextAfter)}</span>
      </div>
    </form> 
  `;

    const form = block.querySelector('.select-vehicle-dropdown-form');
    const yearSelect = block.querySelector(`#${yearId}`);
    const modelSelect = block.querySelector(`#${modelId}`);
    const submitButton = block.querySelector('.select-vehicle-dropdown-submit');
